...
public class A2_SanghyoSeo {
    ...
    static int[] include; // 지금까지 어떤 물건 선택했는지(1이면 선택, 2면 건너뛴 item)
    ...
    public static void knapsack(int i, int profit, int weight, int[] w, int[] p, int W, int n) {
        ...
        if (promising(i, profit, weight, w, p, W, n)) {
            // 해당 item 포함하면 1, include 배열의 0번째 index부터 값을 넣어야 되므로 include[i + 1]이 아닌 include[i] 사용
            include[i] = 1;
            // 왼쪽 노드, i + 1을 불러 recursion을 하는 것이 맞는데, 배열 p에 0번째 요소부터 profit 값이 들어있으므로 ppt처럼
            // p[i + 1]과 w[i + 1]을 더하는 것이 아닌 p[i]값과 w[i]값을 더해주는 것이 맞음
